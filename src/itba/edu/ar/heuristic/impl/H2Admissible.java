...
public class H2Admissible extends Heuristic {
	...
	public int getHeuristicValue(OhnO state) {
		...
		return makeItAdmissible(tokenAndDirections.size());
	}

	private int makeItAdmissible(int size) {
		if (size > 3) {
			return size / 4;
		} else {
			return 1;
	...
	class TokenAndDirection {
		...
		private H2Admissible getOuterType() {
			return H2Admissible.this;
		}
